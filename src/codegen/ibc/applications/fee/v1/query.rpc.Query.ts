...
export class QueryClientImpl implements Query {
  ...
  counterpartyAddress(request: QueryCounterpartyAddressRequest): Promise<QueryCounterpartyAddressResponse> {
    const data = QueryCounterpartyAddressRequest.encode(request).finish();
    const promise = this.rpc.request("ibc.applications.fee.v1.Query", "CounterpartyAddress", data);
    return promise.then(data => QueryCounterpartyAddressResponse.decode(new _m0.Reader(data)));
  }
...
export const createRpcQueryExtension = (base: QueryClient) => {
  ...
  return {
    ...
    counterpartyAddress(request: QueryCounterpartyAddressRequest): Promise<QueryCounterpartyAddressResponse> {
      return queryService.counterpartyAddress(request);
    },
